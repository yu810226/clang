...
struct DOTGraphTraits<const CFG*> : public DefaultDOTGraphTraits {
  ...
  static std::string getEdgeAttributes(const CFGBlock *Node,
                                       GraphTraits<const CFG*>::ChildIteratorType CI,
                                       const CFG *Graph) {
#ifndef NDEBUG
    // Get the statement that causes the branch at the end of the block.
    const Stmt *S = Node->getTerminator().getStmt();
    if (S) {
      CFGBlock::const_succ_iterator SB = Node->succ_begin();
      // If it is an "if" statement that causes the branch
      if (isa<IfStmt>(S)) {
        if (SB[0] == *CI)
          // Paint the arrow to the "then" branch as red with this label:
          return "label = \"then\", color = red, fontcolor = red";
        else
          // This should be the "else" branch, and paint it in blue with
          // this label:
          return "label = \"else\", color = blue, fontcolor = blue";
      }
    }
#endif
    return "";
